...
public class SkyblockerUniversalTextures {
	...
	private static void apply(@Nullable Screen screen) {
		...
		for (Map.Entry<UniversalTextureRule, Identifier> entry : Object2ObjectMaps.fastIterable(ALL_UNIVERSAL_TEXTURES))  {
			...
			if (screen != null && rule.screens().isPresent()) {
				for (ScreenTitleMatcher titleMatcher : rule.screens().get()) {
					if (titleMatcher.test(screen)) APPLICABLE_UNIVERSAL_TEXTURES.put(rule, modelId);
				}
			} else {
	...
	private record UniversalTextureRule(Optional<List<ScreenTitleMatcher>> screens, SkyblockerTexturePredicate[] predicate) implements Predicate<ItemStack> {
		private static final Codec<UniversalTextureRule> CODEC = RecordCodecBuilder.create(instance -> instance.group(
				ScreenTitleMatcher.CODEC.listOf().optionalFieldOf("screens").forGetter(UniversalTextureRule::screens),
	...
	private record ScreenTitleMatcher(Optional<StringMatcher> stringMatcher, Optional<RegexMatcher> regexMatcher) implements Predicate<Screen> {
		private static final Codec<ScreenTitleMatcher> CODEC = RecordCodecBuilder.create(instance -> instance.group(
				StringMatcher.CODEC.optionalFieldOf("stringMatcher").forGetter(ScreenTitleMatcher::stringMatcher),
				RegexMatcher.CODEC.optionalFieldOf("regexMatcher").forGetter(ScreenTitleMatcher::regexMatcher))
				.apply(instance, ScreenTitleMatcher::new));
