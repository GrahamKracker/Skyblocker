...
public class ModelHelper {
	...
	static Set<Identifier> findAuthoritativeModels(Map<Identifier, Resource> models) {
		try {
			Set<CompletableFuture<Set<Identifier>>> futures = new ObjectOpenHashSet<>();

			for (Map.Entry<Identifier, Resource> model : models.entrySet()) {
				futures.add(findReferencedModels(model.getKey(), model.getValue()));
			}
			...
			for (CompletableFuture<Set<Identifier>> future : futures) {
				Set<Identifier> referencedModelsInOverrides = future.join();
	...
	private static CompletableFuture<Set<Identifier>> findReferencedModels(Identifier file, Resource resource) {
